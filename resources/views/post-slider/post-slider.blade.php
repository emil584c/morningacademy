        ...
        <div class="mt-post-slider__top-container">
            <div class="mt-post-slider__text-container">
                @if (!empty($attributes['tagline']))
                    <p class="mt-post-slider__tagline tagline hide-in-admin">{!! $attributes['tagline'] !!}</p>
                @endif
                @if (!empty($attributes['title']))
                    <h2 class="mt-post-slider__title title hide-in-admin">{!! $attributes['title'] !!}</h2>
                @endif
            </div>
                    ...
                    <div class="mt-post-slider__categories-wrapper">
                        <div class="mt-post-slider__categories">
                            <a href="#" class="mt-post-slider__category active"
                                data-term-id="">{{ __('Alle', 'morningtrain') }}</a>
                            @foreach ($terms as $term)
                                <a href="#" class="mt-post-slider__category"
                                    data-term-id="{{ $term->term_id }}">{{ $term->name }}</a>
                            @endforeach
                        </div>
                    </div>
                @endif
                <div @class([
                    'mt-post-slider__navigation' => true,
                ...
                <div class="swiper">
                    <div class="swiper-wrapper">
                        @foreach ($posts as $post)
                            @include('guides/guide-card', $post)
                        @endforeach
                    </div>
                </div>
            @endif
